...
from nis.engines.confidence_engine import decide_visibility
...
nis_aisha = map_db_user_to_user_profile(get_user_dict(aisha))
nis_pref_aisha = map_db_preference_to_match_preference(get_pref_dict(aisha))

candidate_dict = {
    "candidate_id": tariq.id,
    "profile": get_user_dict(tariq)
}
nis_tariq = map_db_candidate_to_candidate_profile(candidate_dict)

print("AISHA EVALUATING TARIQ")
print("HARD FILTER:")
hf = evaluate_hard_filters(nis_aisha, nis_tariq, nis_pref_aisha)
print(hf)

print("\nPSYCHOLOGY:")
psy = evaluate_psychology(nis_aisha, nis_tariq)
print(psy)

print("\nCONFIDENCE:")
c = decide_visibility({"status": "CLEAR"}, hf, {}, psy)
print(c)

...
export default function Preferences() {
  ...
  const goNext = async () => {
    ...
    if (nextIdx < SECTIONS.length) { 
      ...
      try {
        await authPost('/sakinah/match-preference', form);
        setPreferences({ completed: true, data: form }); 
        setShowSuccess(true); 
        setTimeout(() => navigate('/dashboard'), 2000); 
      } catch (err: any) {
        setErrors({ _global: err.message || 'Failed to save preferences' });
      }
    }
  };
      ...
      <div className="bg-white border border-gray-200 rounded-2xl p-7 shadow-sm">
        ...
        {Object.keys(errors).length > 0 && (
          <div className="mt-5 text-red-600 text-sm bg-red-50 p-3 rounded-lg border border-red-200">
            ⚠️ {errors._global || 'Please complete all required fields in this section before continuing.'}
          </div>
        )}

...
import { QuranOpeningOverlay } from '@/features/quran/components/QuranOpeningOverlay';
...
function ActionButtons({ text }: { text: string }) {
  ...
  // When opening the Qur'an reader, play a brief Basmala transition first.
  ...
  // Show the calm gold-halo overlay (no Basmala here), then navigate after ~1.5s.
  const handleOpen = (route: string) => {
    setOpeningRoute(route);
    window.setTimeout(() => navigate(route), 1500);
  };
    ...
    <>
      <div className="flex flex-wrap gap-2 mt-3">
        {actions.map((action) => (
          <button
            key={action.label}
            onClick={() => handleOpen(action.route)}
            className={cn(
      ...
      <AnimatePresence>{openingRoute && <QuranOpeningOverlay showBasmala={false} />}</AnimatePresence>
    </>
  );
}

...
export const SakinahRolePage: React.FC = () => {
  ...
  const [showSeekerOptions, setShowSeekerOptions] = useState(false);
      ...
      <div className="relative z-10 min-h-screen flex flex-col items-center justify-center px-6 py-12">
        <motion.div variants={containerVars} initial="hidden" animate="show" className="w-full max-w-[850px] flex flex-col items-center">
          ...
          <motion.div variants={itemVars} className="grid md:grid-cols-2 gap-8 w-full">
            ...
            <div 
              className={`relative rounded-[32px] overflow-hidden p-[2px] transition-all duration-500 bg-[rgba(255,255,255,0.03)] ${showSeekerOptions ? 'scale-[1.02] shadow-[0_20px_50px_rgba(212,168,83,0.2)] bg-transparent' : 'cursor-pointer hover:-translate-y-3 hover:shadow-[0_20px_50px_rgba(212,168,83,0.2)] hover:bg-transparent group'}`}
              onClick={() => !showSeekerOptions && setShowSeekerOptions(true)}
            >
              {/* Spinning Lighting Effect */}
              <div className={`absolute inset-[-150%] animate-[spin_4s_linear_infinite] transition-opacity duration-700 ${showSeekerOptions ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`} style={{ background: 'conic-gradient(from 90deg, transparent 0%, transparent 75%, var(--sk-gold) 100%)' }} />
              ...
              <div className="relative h-[400px] w-full bg-[#0A0E16] rounded-[30px] p-10 flex flex-col items-center text-center z-10 transition-all duration-500 overflow-hidden">
                <div className={`absolute inset-0 bg-gradient-to-b from-[rgba(212,168,83,0.04)] to-transparent transition-opacity duration-700 ${showSeekerOptions ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`} />
                
                <div className="relative z-10 flex-1 flex flex-col items-center justify-start w-full">
                  <div className={`w-24 h-24 mb-6 rounded-full flex items-center justify-center transition-all duration-500 ${showSeekerOptions ? 'bg-[rgba(212,168,83,0.08)] border border-[var(--sk-gold)] shadow-[0_0_30px_rgba(212,168,83,0.3)] scale-110' : 'bg-[rgba(255,255,255,0.02)] border border-[rgba(255,255,255,0.05)] group-hover:scale-110 group-hover:border-[var(--sk-gold)] group-hover:shadow-[0_0_30px_rgba(212,168,83,0.3)]'}`}>
                    <span className="text-[38px] text-[var(--sk-gold)]">👤</span>
                  </div>
                  <h2 className={`font-display text-[32px] mb-3 transition-colors ${showSeekerOptions ? 'text-[var(--sk-gold)]' : 'text-[var(--sk-ink)] group-hover:text-[var(--sk-gold)]'}`}>I am a Seeker</h2>
                  ...
                  <AnimatePresence>
                    {showSeekerOptions && (
                      <motion.div 
                        key="seeker-options-container"
                        initial={{ opacity: 0, y: 10 }} 
                        animate={{ opacity: 1, y: 0 }} 
                        exit={{ opacity: 0, y: 10 }}
                        className="w-full flex flex-col gap-3 mt-4"
                      >
                        <button onClick={(e) => { e.stopPropagation(); handleSeekerRegister(); }} className="w-full py-3.5 bg-gradient-to-r from-[var(--sk-gold)] to-[#E8C97A] hover:from-[#E8C97A] hover:to-[#b98b39] text-[#0A0E16] rounded-2xl transition-all duration-300 shadow-[0_0_20px_rgba(212,168,83,0.3)] text-[14px] font-bold tracking-wide">
                          Register New Account
                        </button>
                        <button onClick={(e) => { e.stopPropagation(); handleSeekerLogin(); }} className="w-full py-3.5 bg-[rgba(255,255,255,0.03)] hover:bg-[rgba(212,168,83,0.08)] border border-[rgba(255,255,255,0.08)] hover:border-[var(--sk-gold)] text-[var(--sk-ink)] hover:text-[var(--sk-gold)] rounded-2xl transition-all duration-300 text-[14px] font-medium tracking-wide">
                          Login to Dashboard
                        </button>
                      </motion.div>
                    )}
                  ...
                  <AnimatePresence>
                    {showWaliOptions && (
                      <motion.div 
                        ...
                        className="w-full flex flex-col gap-3 mt-4"
                      >
                        <button onClick={(e) => { e.stopPropagation(); handleWaliLogin(); }} className="w-full py-3.5 bg-[rgba(255,255,255,0.03)] hover:bg-[rgba(212,168,83,0.08)] border border-[rgba(255,255,255,0.08)] hover:border-[var(--sk-gold)] text-[var(--sk-ink)] hover:text-[var(--sk-gold)] rounded-2xl transition-all duration-300 text-[14px] font-medium tracking-wide">
                          Login to Dashboard
                        </button>
                        <button onClick={(e) => { e.stopPropagation(); handleWaliRegister(); }} className="w-full py-3.5 bg-gradient-to-r from-[var(--sk-gold)] to-[#E8C97A] hover:from-[#E8C97A] hover:to-[#b98b39] text-[#0A0E16] rounded-2xl transition-all duration-300 shadow-[0_0_20px_rgba(212,168,83,0.3)] text-[14px] font-bold tracking-wide">
          ...
          <motion.div variants={itemVars} className="mt-16">
            <button onClick={() => navigate('/matrimony/begin')} className="text-[13px] text-[var(--sk-ink-faint)] hover:text-[var(--sk-gold)] transition-colors py-2 px-4 uppercase tracking-[0.2em] font-mono">
              ← Back
            </button>
          </motion.div>

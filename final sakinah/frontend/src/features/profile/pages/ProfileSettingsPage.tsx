...
import { Sparkle, Brain, CaretRight } from '@phosphor-icons/react';
...
export function ProfileSettingsPage() {
  ...
  return (
    <div className="min-h-[calc(100dvh-60px)] bg-transparent">
      ...
      <RevealOnScroll className="px-4 mb-4">
      ...
      {/* ── What Raya Remembers (3D memory graph) ── */}
      <div className="px-4 mb-4">
        <button
          onClick={() => navigate('/profile/memory')}
          className="w-full p-4 rounded-xl border border-[#D4A853]/25 flex items-center gap-3"
          style={{ background: 'linear-gradient(135deg, rgba(122,162,255,0.08), rgba(95,208,196,0.03))' }}
        >
          <div className="w-11 h-11 shrink-0 rounded-xl flex items-center justify-center bg-[#7aa2ff]/12 border border-[#7aa2ff]/25">
            <Brain size={22} weight="duotone" className="text-[#7aa2ff]" />
          </div>
          <div className="text-left flex-1">
            <p className="text-[#F5E8C7] text-sm font-semibold">What Raya Remembers</p>
            <p className="text-[#5C5749] text-xs mt-0.5">Explore your relationships & memories as a living 3D map</p>
          </div>
          <CaretRight size={18} className="text-[#5C5749] shrink-0" />
        </button>
      </div>

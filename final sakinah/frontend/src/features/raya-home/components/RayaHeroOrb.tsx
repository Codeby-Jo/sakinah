...
export function RayaHeroOrb({ className }: { className?: string }) {
  const flight = useMemo(() => makeFlight(), []);
  return (
    <motion.div
      className={className ? `rho-wrap ${className}` : 'rho-wrap'}
      aria-hidden="true"
...
@media (max-width:760px){.rho-wrap{margin-bottom:16px;}.rho-orb{width:74px;height:74px;}.rho-core{width:62px;height:62px;}}

...
export async function fetchTafsir(verseKey: string, tafsirId = 169): Promise<string | null> {
  try {
    ...
    return text ? formatTafsirHtml(text) : null;
  } catch {
    return null;
  }
}
...
/**
 * Tafsir-specific HTML formatter.
 *
 * cleanHtml (above) flattens everything to one plain-text line — correct for
 * short translation strings, but it's exactly what turned the tafsir into an
 * unreadable wall of text. Tafsir from Quran.com arrives as rich HTML with
 * section headings, paragraphs and bold emphasis, so here we KEEP that
 * structure and let the reader render it like Quran.com.
 *
 * Safety: we never trust the source markup. We rebuild the tree from a fixed
 * whitelist of structural tags, DROP every attribute, and escape all text — so
 * no href / onclick / style / script / footnote markers can survive. The result
...
function formatTafsirHtml(input: string): string {
  // No DOM available (SSR / tests) → fall back to the safe plain-text cleaner.
  if (typeof window === 'undefined' || typeof window.DOMParser === 'undefined') {
    return cleanHtml(input);
  }

  const doc = new DOMParser().parseFromString(input, 'text/html');
  // Footnote markers and any script/style: remove the element AND its content.
  doc.querySelectorAll('sup, script, style').forEach((el) => el.remove());
  ...
  const walk = (node: Node): string => {
    ...
    node.childNodes.forEach((child) => {
      if (child.nodeType === 3 /* TEXT_NODE */) {
        ...
        const inner = walk(el);
        if (TAFSIR_ALLOWED_TAGS.has(tag)) {
          const t = tag.toLowerCase();
          out += `<${t}>${inner}</${t}>`;
        } else {
          out += inner; // unknown tag → keep its text, drop the wrapper
        }
      }
    });
    return out;
  ...
  return walk(doc.body)
    // Drop empty paragraphs/divs Quran.com leaves behind.
    .replace(/<(p|div)>\s*(?:&nbsp;| |\s|<br\/>)*<\/\1>/gi, '')
    // Collapse long runs of <br/> down to a single paragraph break.

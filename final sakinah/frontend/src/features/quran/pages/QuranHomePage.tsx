...
import { QuranOpeningOverlay } from '../components/QuranOpeningOverlay';
...
/**
 * Reports which way the user is scrolling: 'down' or 'up'.
...
export function QuranHomePage() {
  ...
  // Basmala transition (~1.25s) shown when ENTERING the Quran tab from outside the
  // Quran section: on a fresh load / refresh (no previous route) OR an in-app nav
  // from a non-Quran route (e.g. Prayer → Quran). NOT when returning from a Quran
  // sub-page (Mushaf, Read, …). MainLayout records the route during render, so
  // `previousPath` holds where we came from even though this page is lazy-loaded.
  const [showIntro, setShowIntro] = useState(() => {
    const from = navHistory.previousPath;
    return !from || !from.startsWith('/quran');
  });
  useEffect(() => {
    if (!showIntro) return;
    const t = window.setTimeout(() => setShowIntro(false), 1250);
  ...
  return (
    <div className="min-h-[calc(100dvh-60px)] bg-transparent">
      ...
      {/* Navigation cards — DIRECTION-AWARE SCROLL REVEAL (repeats every time):
          each card fades into view whenever it scrolls onto the screen, entering
          FROM the direction you're scrolling.
          - `whileInView` plays the animation when the card enters the viewport.
          - `initial.y` follows scroll direction: scrolling DOWN → cards start below
            (+24) and rise up; scrolling UP → cards start above (-24) and come down,
            so the motion always matches the way you're moving.
          - No `once` => it re-plays every time (resets off-screen, fades in again).
          - `amount: 0.2` => trigger once ~20% of the card is visible.
          - Movement + hover are BOTH handled by Framer Motion (`whileHover`) so
            nothing fights over the CSS `transform` — that fixes the scroll lag the
            old Tailwind `hover:scale-[1.01] transition-transform` caused.
          Automatically responsive: on a phone (where ~1 card fits) the other ~11
          all reveal as you scroll; on desktop the first few animate on load. */}
      <div className="px-4 space-y-3">
        {SECTIONS.map((section) => (
          <motion.button
            ...
            className={`w-full flex items-center gap-4 p-4 rounded-xl bg-gradient-to-r ${section.gradient} border ${section.borderColor} text-left`}
          >
      ...
      {/* Basmala transition shown when the Quran tab opens. */}
      <AnimatePresence>{showIntro && <QuranOpeningOverlay />}</AnimatePresence>
    </div>
  );
}

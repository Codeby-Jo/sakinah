...
export function EimLessonPage() {
  ...
  return (
    // pb below lg clears BOTH stacks: the lesson's own action bar (~4.25rem)
    // AND EimShell's bottom tab bar (h-16 + safe-area-inset-bottom), which is
    // shown below lg. At lg+ the tab bar is hidden, so only the action bar needs
    // clearance.
    <div className="min-h-[calc(100dvh-60px)] bg-[#0C0F15]/70 backdrop-blur-md pb-[calc(9rem+env(safe-area-inset-bottom,0px))] lg:pb-32">
      {/* Top bar */}
      ...
      {/* Mobile sidebar drawer. Portaled to <body> so it escapes EimShell's
          <main> (z-5) stacking context — otherwise the z-30 top bar / bottom nav
          paint over the drawer's header (incl. its close button) and footer. */}
      ...
      {/* Sticky bottom nav bar.
          EIM lives in its own EimShell, whose bottom tab bar (h-16, z-30) is
          shown below lg. Below lg we sit ABOVE that tab bar (offset by its 4rem
          height) so the Prev/Next/Take-quiz controls aren't covered. At lg+ the
          tab bar is hidden, so we anchor flush to bottom-0. */}
      <div
        className="fixed left-0 right-0 z-20 backdrop-blur-md bg-[rgba(15,23,36,0.92)] border-t border-[rgba(212,168,83,0.14)] bottom-16 lg:bottom-0"
      >

...
interface EimState {
  ...
  // ── First-run "how to exit the mini-app" hint ────────────────────────────
  /** True once the user has dismissed the one-time coachmark that points at
   *  the Exit control. EIM is a full-screen mini-app that hides the global
   *  Zaryah+ chrome, so first-timers don't realise they must Exit to get back. */
  exitHintSeen: boolean;
  markExitHintSeen: () => void;
}
...
export const useEimStore = create<EimState>()(
  persist(
    (set, get) => ({
      ...
      exitHintSeen: false,
      markExitHintSeen: () => set((s) => (s.exitHintSeen ? s : { exitHintSeen: true })),
    }),
    {
      name: 'zaryah:eim',
      ...
      partialize: (state) => ({
        ...
        exitHintSeen: state.exitHintSeen,
      }),
    },
  ),
);

...
export function MainLayout() {
  ...
  // Record the route during render (runs at navigation time, before lazy route
  // chunks resolve) so pages can tell which route the user came from. No-op when
  // the path is unchanged, so repeated / StrictMode re-renders are harmless.
    ...
    <div className="fixed inset-0 overflow-hidden bg-[#06080D] text-[#F5E8C7] font-sans">
      ...
      <header className="fixed top-0 inset-x-0 h-[60px] z-40 flex items-center justify-between px-[18px] sm:px-[26px] backdrop-blur-[14px] bg-gradient-to-b from-[#06080D]/85 to-transparent">
        <div className="flex items-center gap-3">
          ...
          >
            {/* `!h-*` required — src/index.css has an unlayered `img { height: auto }`
                that otherwise overrides Tailwind and balloons the logo. */}
      ...
      <main ref={mainRef} className="fixed inset-x-0 bottom-0 top-[60px] z-[5] overflow-y-auto">
        {/* Tab transition — a calm, dignified "settle into place": each page
            fades in while gently rising 16px. Chosen to match the serene
            Raya-universe aesthetic (sakīnah) rather than a flashy slide. The
            `key` replays the animation on every route change. */}
        <motion.div

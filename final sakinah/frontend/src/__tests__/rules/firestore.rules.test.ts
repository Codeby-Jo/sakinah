...
// ────────────────────────────────────────────────────────────────────
// /users/{uid}/memory_edges — owner-read, server-only-write (Raya KG memory)
// ────────────────────────────────────────────────────────────────────

describe('/users/{uid}/memory_edges — owner-read, server-only writes', () => {
  it('owner can read their own memory edge', async () => {
    await testEnv.withSecurityRulesDisabled(async (admin) => {
      await setDoc(doc(admin.firestore(), 'users/alice/memory_edges/e1'), {
        subject: 'user', predicate: 'LIVES_IN', object: 'enc:v1:xxx', valid_to: null,
      });
    });
    const alice = testEnv.authenticatedContext('alice');
    await assertSucceeds(getDoc(doc(alice.firestore(), 'users/alice/memory_edges/e1')));
  });

  it('owner CANNOT write their own edge directly (Admin-SDK only)', async () => {
    const alice = testEnv.authenticatedContext('alice');
    await assertFails(
      setDoc(doc(alice.firestore(), 'users/alice/memory_edges/forged'), {
        subject: 'user', predicate: 'LIVES_IN', object: 'Madrid',
      })
    );
  });

  it('owner CANNOT update their own edge (e.g. tamper with valid_to)', async () => {
    await testEnv.withSecurityRulesDisabled(async (admin) => {
      await setDoc(doc(admin.firestore(), 'users/alice/memory_edges/e1'), {
        subject: 'user', predicate: 'LIVES_IN', object: 'enc:v1:xxx', valid_to: null,
      });
    ...
    const alice = testEnv.authenticatedContext('alice');
    await assertFails(
      updateDoc(doc(alice.firestore(), 'users/alice/memory_edges/e1'), { valid_to: null, confidence: 1 })
    );
  });

  it('owner CANNOT delete their own edge (forget is server-side)', async () => {
    await testEnv.withSecurityRulesDisabled(async (admin) => {
      await setDoc(doc(admin.firestore(), 'users/alice/memory_edges/e1'), { subject: 'user' });
    });
    const alice = testEnv.authenticatedContext('alice');
    const { deleteDoc } = await import('firebase/firestore');
    await assertFails(deleteDoc(doc(alice.firestore(), 'users/alice/memory_edges/e1')));
  });

  it('non-owner CANNOT read another user’s edges (PII isolation)', async () => {
    await testEnv.withSecurityRulesDisabled(async (admin) => {
      await setDoc(doc(admin.firestore(), 'users/bob/memory_edges/e1'), { subject: 'user' });
    });
    const alice = testEnv.authenticatedContext('alice');
    await assertFails(getDoc(doc(alice.firestore(), 'users/bob/memory_edges/e1')));
  });

  it('unauthenticated CANNOT read edges', async () => {
    await testEnv.withSecurityRulesDisabled(async (admin) => {
      await setDoc(doc(admin.firestore(), 'users/alice/memory_edges/e1'), { subject: 'user' });
    });
    const anon = testEnv.unauthenticatedContext();
    await assertFails(getDoc(doc(anon.firestore(), 'users/alice/memory_edges/e1')));
  });
});

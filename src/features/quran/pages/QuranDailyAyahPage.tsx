...
export function QuranDailyAyahPage() {
  ...
  // The tafsir is now structured HTML (see formatTafsirHtml). We render that
  // HTML for the reader, but Raya/guidance consumers want plain text — strip
  // tags to a single clean line for them (tags would just be noise to the LLM).
  const tafsirPlain = tafsir ? tafsir.replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').trim() : null;

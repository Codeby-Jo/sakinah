...
interface SettingsPanelProps {
  ...
  onTranslationFontSizeChange: (n: number) => void;
  onTafsirFontSizeChange: (n: number) => void;
}
...
export function SettingsPanel({
  ...
  arabicFontSize, arabicLineHeight, translationFontSize, tafsirFontSize,
  ...
  onTranslationFontSizeChange, onTafsirFontSizeChange,
}: SettingsPanelProps) {
  return (
    <AnimatePresence>
      {open && (
        ...
        >
          ...
          <div className="mt-3 pt-3 border-t border-[#F5E8C7]/10 space-y-2">
            ...
            <label className="flex items-center justify-between text-xs text-[#C9C0A8]">
              <span>Translation size · {translationFontSize}px</span>
              <input
                type="range"
                min={12}
                max={24}
            ...
            <label className="flex items-center justify-between text-xs text-[#C9C0A8]">
              <span>Tafsir size · {tafsirFontSize}px</span>
              <input
                type="range"
                min={11}
                max={22}
                value={tafsirFontSize}
                onChange={(e) => onTafsirFontSizeChange(parseInt(e.target.value))}
                className="accent-amber-500 w-40"
              />
            </label>
          </div>

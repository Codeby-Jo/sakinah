  ...
  const onSubmit = async (data: RegisterFormData) => {
    ...
    try {
      ...
      const res = await registerSakinah({
        email: data.email,
        password: data.password,
        full_name: 'Seeker', // Temporary filler until profile creation
        age: 18,             // Temporary filler
      });
        ...
        >
          ...
          <div className="mt-8 text-center">
            <p className="text-white/40 text-sm">
              ...
              <Link to="/matrimony/login" className="text-[#D4AF37] font-medium hover:text-[#F5D77A] transition-colors ml-1">
                Log In
              </Link>
            </p>
          </div>

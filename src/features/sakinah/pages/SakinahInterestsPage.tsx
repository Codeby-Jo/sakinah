    ...
    <SakinahLayout>
      <div className="p-6 md:p-10 max-w-[1000px] mx-auto min-h-screen">
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
          <SakinahHeader title="Interests" subtitle="Manage your connections and requests" onBack={() => navigate('/dashboard')} />
        </motion.div>
              ...
              {items.length === 0 ? (
                <div className="max-w-[400px] mx-auto text-center mt-12">
                  ...
                  {activeTab === 'Received' || activeTab === 'Sent' ? (
                    <button onClick={() => navigate('/matches')} className="px-6 py-3 bg-[var(--sk-gold)] text-[#0A0E16] rounded-full text-[14px] font-medium hover:bg-[#E8C97A] transition-colors shadow-[0_0_20px_rgba(212,168,83,0.3)]">
                      Discover Matches
                    </button>
                  ) : null}
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {items.map(item => (
                    <div key={item.id} className="sk-card p-5 hover:border-[var(--sk-gold)] transition-all cursor-pointer group" onClick={() => navigate('/matches')}>
                      ...
                      <div className="mt-4 pt-4 border-t border-[rgba(255,255,255,0.05)] flex gap-2">
                        {!isWaliViewOnly ? (
                          <>
                            ...
                            {activeTab === 'Accepted' && (
                              <button onClick={(e) => { e.stopPropagation(); navigate('/chat'); }} className="w-full py-1.5 bg-[var(--sk-gold)] text-[#0A0E16] rounded-full text-[12px] font-medium hover:bg-[#E8C97A] transition-colors shadow-[0_0_15px_rgba(212,168,83,0.2)]">Message Now</button>
                            )}
                          </>
                        ) : (

...
const MatchCard = React.memo(({ 
  ...
  >
    ...
    <div className="p-4 border-t border-[rgba(255,255,255,0.05)] bg-[#0C111A] flex gap-3 z-10 relative">
      ...
      <button className="flex-1 py-2.5 rounded-xl border border-[rgba(212,168,83,0.3)] text-[var(--sk-gold)] text-[13px] font-medium hover:bg-[rgba(212,168,83,0.1)] hover:shadow-[0_0_15px_rgba(212,168,83,0.15)] transition-all duration-300">
        View Profile
      </button>
      <button 
        onClick={onInterest} 
...
export const SakinahMatchesPage: React.FC = () => {
  ...
  const handleInterest = useCallback(async (id: string) => {
    ...
    try {
      ...
      const activeCount = convos.filter((c: any) => c.status !== 'closed').length;
  ...
  const renderedMatches = useMemo(() => {
    return matches.map(m => (
      <MatchCard
        ...
        onReport={() => setReportingProfile(m.name)}
      />
    ));
  }, [matches, isWali, interested, handlePass, handleInterest]);

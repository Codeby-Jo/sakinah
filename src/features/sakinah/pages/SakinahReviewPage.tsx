...
import { CheckCircle, WarningCircle, PencilSimple, ShieldCheck, User, Heart, IdentificationCard, EnvelopeSimple, Phone } from '@phosphor-icons/react';
...
export const SakinahReviewPage: React.FC = () => {
  ...
  const { auth, kyc, profile, preferences, setOnboardingComplete } = useOnboarding();
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Set a mock token so they are recognized as logged in during the mock flow
      localStorage.setItem('sakinah_token', 'mock_token_after_onboarding');
      setProgress({ review_completed: true });
  ...
  const SectionHeader = ({ title, editPath, icon: Icon }: any) => (
    <div className="flex justify-between items-center mb-4 border-b border-[#D4AF37]/10 pb-3">
      <h3 className="text-[#F5D77A] text-lg font-serif flex items-center gap-2">
    ...
    >
      ...
      >
        ...
        <div className="bg-[#111826] border border-[#D4AF37]/30 rounded-3xl p-6 md:p-8 flex flex-col md:flex-row items-center gap-6 shadow-[0_0_30px_rgba(212,175,55,0.1)] relative overflow-hidden">
          ...
          <div className="w-24 h-24 rounded-full border-2 border-[#D4AF37] p-1 shrink-0 bg-[#0B1020]">
            <div className="w-full h-full rounded-full bg-gradient-to-br from-[#D4A853] to-[#A37B31] flex items-center justify-center text-[#0A0E16] text-[32px] font-serif font-bold shadow-[inset_0_0_20px_rgba(0,0,0,0.2)]">
              {kyc.aadhaarName ? kyc.aadhaarName.charAt(0).toUpperCase() : 'S'}
            </div>
          </div>
          <div className="flex-1 text-center md:text-left z-10">
            <h2 className="text-[28px] font-serif text-[#F5D77A] mb-2">{kyc.aadhaarName || 'User Profile'}</h2>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-[#0B1020]/80 border border-[rgba(212,175,55,0.15)] rounded-3xl p-6 md:p-8 relative overflow-hidden backdrop-blur-xl">
             ...
             <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mt-4">
                <div className="flex items-center gap-3">
                   ...
                   <div>
                     ...
                     <div className="text-[14px] text-white/90">{auth.email || 'Not provided'}</div>
                   </div>
                </div>
                <div className="flex items-center gap-3">
                   <div className="w-10 h-10 rounded-full bg-[#D4AF37]/10 flex items-center justify-center text-[#D4AF37]">
                   ...
                   <div>
                     <div className="text-[10px] text-[#F5D77A]/50 uppercase tracking-widest font-medium">Phone Number</div>
                     <div className="text-[14px] text-white/90">{auth.phone || 'Not provided'}</div>
                   </div>
                </div>
                <div className="col-span-1 sm:col-span-2 pt-4 mt-2 border-t border-white/5">
                   <div className="flex justify-between items-center bg-[#050816] p-4 rounded-xl border border-white/5">
                      <div className="flex flex-col gap-1">
                         <span className="text-[10px] text-[#F5D77A]/50 uppercase tracking-widest font-medium">Identity Status</span>
                         <span className="text-[13px] text-white">{kyc.aadhaarName || 'Verified User'}</span>
                         {kyc.aadhaarNumber && <span className="text-[11px] text-white/40">ID: {kyc.aadhaarNumber.replace(/\d{8}/, '••••••••')}</span>}
                      </div>

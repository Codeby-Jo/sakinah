...
import { Card, EmptyState, RevealOnScroll, SectionTitle, SkeletonRows } from './ui';
  ...
  return (
    <div>
      ...
      {!hasReviewA ? (
        ...
        <RevealOnScroll className="space-y-3 mb-8">
          {report?.summary && (
            <Card className="p-5">
      ...
      {people.length > 0 && (
        <>
          ...
          <RevealOnScroll className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-8">
            {people.map((p, i) => (

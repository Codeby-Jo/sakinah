...
import zaryahWordmark from '@/assets/zaryah-wordmark.png';
...
export function RayaGatewayPage() {
  ...
  // Public landing: guests see the calm hero but anything that needs an account
  // (sending a message, opening a feature) routes them to sign up.
  const isAuthenticated = useAuthStore((s) => s.state.type === 'authenticated');
  const requireAuth = useCallback(() => navigate('/signup'), [navigate]);
  ...
  const handleSend = useCallback((text: string) => {
    const t = text.trim();
    if (!t) return;
    if (!isAuthenticated) { requireAuth(); return; } // guests must sign up to chat
    if (rayaSend(t) === 'nav') return; // a nav-verb command already navigated away
    isUserScrolledUp.current = false;
    sendUserMessage(t);
  }, [isAuthenticated, requireAuth, rayaSend, sendUserMessage]);
  ...
  const goFeature = (route: string) => {
    if (!isAuthenticated) { requireAuth(); return; } // guests must sign up to enter a feature
    playWarp({
    ...
    <div className="fixed inset-0 overflow-hidden bg-[#06080D] text-[#F5E8C7] font-sans">
      ...
      <header className="fixed top-0 inset-x-0 h-[60px] z-40 flex items-center justify-between px-[18px] sm:px-[26px] backdrop-blur-[14px] bg-gradient-to-b from-[#06080D]/85 to-transparent">
          ...
          <AnimatePresence mode="wait" initial={false}>
            {chatting ? (
              ...
              <motion.button
                ...
                aria-label="Zaryah+ home"
                className="flex items-center hover:opacity-80 transition-opacity"
              >
                {/* `!h-*` (important) is required: src/index.css has an unlayered
                    `img { height: auto }` rule that otherwise beats Tailwind's
                    utility layer and lets the logo balloon to full width. */}
                <img src={zaryahWordmark} alt="Zaryah+" className="!h-7 sm:!h-8 w-auto" />
              </motion.button>
            )}
          </AnimatePresence>
        </div>
        ...
        <AnimatePresence mode="wait" initial={false}>
          {chatting ? (
            ...
            /* Guest: sign in / sign up entry points */
            <motion.div
              key="authcta"
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
            ...
            >
              <button
                ...
                className="text-[13px] text-[#C9C0A8] hover:text-[#F5E8C7] px-3 py-[7px] rounded-full transition-colors"
              >
                Sign in
              </button>
              <button
                onClick={() => navigate('/signup')}
                className="text-[13px] font-medium text-[#1a1206] bg-gradient-to-br from-[#E8C97A] to-[#D4A853] hover:brightness-110 px-4 py-[7px] rounded-full transition-all"
              >
                Sign up
              </button>
            </motion.div>
      ...
      <main className="fixed inset-x-0 bottom-0 top-[60px] z-[5] flex flex-col">
        <div className="relative h-full max-w-[760px] w-full mx-auto flex flex-col px-4 sm:px-7 min-h-0">
            ...
            <AnimatePresence initial={false}>
              {chatting ? (
                ...
                <motion.div
                  ...
                  exit={{}}
                  transition={{ duration: 0.85, ease: SLOW_EASE }}
                  /* Auto margins (not justify-center) centre the hero, so when a
                     short mobile viewport can't fit it the column scrolls from the
                     top instead of clipping the orb behind the topbar. */
                  className="absolute inset-0 z-20 pointer-events-none flex flex-col overflow-y-auto overflow-x-hidden sm:overflow-visible py-2"
                >
                  {/* Orb is a sibling of the text so it keeps its own (no-fade)
                      fly-off exit while the text gently fades up and out. */}
                  <RayaHeroOrb className="mt-auto shrink-0" />
                  <motion.div
                    /* pointerEvents flips off on exit so the fading hero never
                       blocks the chat thread easing in beneath it. */
                    exit={{ opacity: 0, y: -28, pointerEvents: 'none' }}
                    transition={{ duration: 0.7, ease: SLOW_EASE }}
                    className="text-center pb-1 mb-auto shrink-0 pointer-events-auto"
                  >
                    <div className="font-arabic text-[19px] sm:text-[21px] text-[#E8C97A] opacity-85 mb-4 sm:mb-6 tracking-[0.5px]">بِسْمِ اللهِ</div>
                    <div className="font-arabic text-[13px] sm:text-[15px] text-[#8A8270] tracking-[3px] uppercase mb-3 sm:mb-4 flex items-center justify-center gap-3 before:content-[''] before:w-[30px] before:h-px before:bg-gradient-to-r before:from-transparent before:to-[#F5E8C7]/10 after:content-[''] after:w-[30px] after:h-px after:bg-gradient-to-l after:from-transparent after:to-[#F5E8C7]/10">
                      رايا · Raya
                    </div>
                    <h1 className="font-display font-normal leading-[1.12] tracking-[0.2px] mb-3 sm:mb-[18px] text-[#F5E8C7] text-balance" style={{ fontSize: 'clamp(24px,4vw,44px)' }}>
                      Assalamu alaikum. I&rsquo;m Raya — <br />
                      <em className="not-italic text-[#E8C97A] italic">
                        the world&rsquo;s first{' '}
                        <span className="whitespace-nowrap">Islamic super agent.</span>
                      </em>
                    </h1>
                    <p className="text-[14.5px] sm:text-[16px] leading-[1.65] text-[#C9C0A8] max-w-[560px] mx-auto mb-5 sm:mb-[26px] font-light px-1">
                      The first intelligence built on <b className="text-[#E8C97A] font-medium">shukr</b> — and your personal twin for faith and life.
                    </p>
                    <div className="flex flex-wrap gap-2 sm:gap-2.5 justify-center max-w-[620px] mx-auto pointer-events-auto">
                      {HERO_CHIPS.map((c) => (
                        <button
                          key={c.id}
                          onClick={() => goFeature(c.route)}
                          className="flex items-center gap-[7px] text-[12px] sm:text-[13px] text-[#C9C0A8] font-normal px-3 sm:px-[15px] py-2 sm:py-[9px] border border-[#F5E8C7]/10 rounded-full bg-[#F5E8C7]/[0.025] hover:border-[#D4A853]/40 hover:bg-[#D4A853]/[0.07] hover:text-[#F5E8C7] transition-all hover:-translate-y-px"
                        >
          ...
          {/* Composer rail */}
          <div className="shrink-0 pb-[max(14px,env(safe-area-inset-bottom))] sm:pb-[22px] pt-2.5">
            {quranAnchor && (
              <QuranAnchorPill
                anchor={quranAnchor}
            ...
            {chatting ? (
              ...
              <div className="text-center text-[11px] sm:text-[11.5px] text-[#4A4639] mt-2 sm:mt-[11px] tracking-[0.3px] px-2">

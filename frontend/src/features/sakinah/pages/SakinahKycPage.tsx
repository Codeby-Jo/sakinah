...
export const SakinahKycPage: React.FC = () => {
  ...
  const { updateKyc, setKycCompleted, userType } = useOnboarding();
  ...
  const isLocked = isSeeker && !isLivenessPassed;
  ...
  const handleProfilePhotoChange = (f: File | null) => {
    setProfilePhoto(f);
    if (!f) setProfilePhotoError('Profile Photo is required.');
    else {
  ...
  const handleSubmit = async () => {
    ...
    if (isSeeker && !isLivenessPassed) {
      setProfilePhotoError('Liveness incomplete');
      hasError = true;
    } else if (!isSeeker && (!profilePhoto || profilePhotoError)) {
      setProfilePhotoError('Valid Profile Photo is required.');
      hasError = true;
    }
    ...
    setProgress({ role: currentRole, account_completed: true, kyc_completed: true });
        ...
        <div className="flex flex-col gap-6 w-full max-w-4xl mx-auto">
          ...
          >
            ...
            <div className="space-y-10">
              {/* Profile Photo / Liveness */}
              {isSeeker ? (
                <div className="flex flex-col gap-4">
                  <label className="text-[11px] text-[#D4AF37] uppercase tracking-[0.2em] font-semibold">1. Liveness Verification</label>
                ...
                <div className="flex flex-col gap-4">
                  <label className="text-[11px] text-[#D4AF37] uppercase tracking-[0.2em] font-semibold">1. Clear Profile Photo</label>
                  <div className="max-w-xs">
                    <FileUpload label="Profile Photo" file={profilePhoto} error={profilePhotoError} onChange={handleProfilePhotoChange} required />
                    {profilePhotoError && <p className="text-[11px] text-red-400 mt-2 pl-1">{profilePhotoError}</p>}
                  </div>
                </div>
              )}

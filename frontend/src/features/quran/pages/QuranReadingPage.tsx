...
export function QuranReadingPage() {
  ...
  // Translation + tafsir text size — independent of Arabic, also persisted.
  const [translationFontSize, setTranslationFontSize] = useState<number>(() => {
    const n = parseInt(localStorage.getItem('quran_translation_font_size') ?? '14', 10);
    return Number.isFinite(n) && n >= 12 && n <= 24 ? n : 14;
  });
  const [tafsirFontSize, setTafsirFontSize] = useState<number>(() => {
    const n = parseInt(localStorage.getItem('quran_tafsir_font_size') ?? '13', 10);
    return Number.isFinite(n) && n >= 11 && n <= 22 ? n : 13;
  });
  ...
  useEffect(() => { localStorage.setItem('quran_translation_font_size', String(translationFontSize)); }, [translationFontSize]);
  useEffect(() => { localStorage.setItem('quran_tafsir_font_size', String(tafsirFontSize)); }, [tafsirFontSize]);
  ...
  return (
    ...
    >
      ...
      <SettingsPanel
        ...
        onTafsirFontSizeChange={setTafsirFontSize}
      />
